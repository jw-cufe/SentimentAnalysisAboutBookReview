...
blue_high=[]
for f in os.listdir('/Users/Lisiyan/Documents/项目/hillarytrump演示文稿/blue_high/'):
    for line in open("/Users/Lisiyan/Documents/项目/hillarytrump演示文稿/blue_high/"+f):
        line=line.strip()
        blue_high.append(line)
blue_high=[s.split() for s in blue_high]
blue_low=[]
for f in os.listdir('/Users/Lisiyan/Documents/项目/hillarytrump演示文稿/blue_low/'):
    for line in open("/Users/Lisiyan/Documents/项目/hillarytrump演示文稿/blue_low/"+f):
        line=line.strip()
        blue_low.append(line)
blue_low=[s.split() for s in blue_low]
resample=len(blue_low)//len(blue_high)
text=np.array(blue_high*resample+blue_low)
y=np.array([0 for _ in blue_high*resample]+[1 for _ in blue_low])
...
blue_high_counts=Counter(itertools.chain(*blue_high))
blue_low_counts=Counter(itertools.chain(*blue_low))
...
for i in range(100):
    ...
    tmp["high"]=blue_high_counts[tmp["feature"]]
    tmp["low"]=blue_low_counts[tmp['feature']]
